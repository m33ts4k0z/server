...
router.post(
    ...
    async (req: Request, res: Response) => {
        ...
        const guilds = await Guild.find({ where: { owner_id: user_id } });
        await Promise.all(
            guilds.map(async (guild) => {
                const members = await Member.find({
                    where: { guild_id: guild.id, id: Not(user_id) },
                ...
                if (sortedMembers.length === 0) {
                    ...
                    const stickers = await Sticker.find({ where: { guild_id: guild.id, user_id } });
                    await Promise.all(
                    ...
                    const emojis = await Emoji.find({ where: { guild_id: guild.id, user_id } });
                    await Promise.all(
        ...
        const members = await Member.find({ where: { id: user_id } });
        await Promise.all([...members.map((member) => Member.removeFromGuild(member.id, member.guild_id))]);
        await UserSettingsProtos.delete({ user_id });
