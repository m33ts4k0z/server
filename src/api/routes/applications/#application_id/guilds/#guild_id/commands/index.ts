...
router.get("/", route({}), async (req: Request, res: Response) => {
    const { application_id, guild_id } = req.params as { application_id: string; guild_id: string };
    ...
    const guildExists = await Guild.exists({ where: { id: guild_id } });
    ...
    if (!(await Member.exists({ where: { id: application_id, guild_id } }))) {
        res.status(401).send({ code: 401, message: "Missing Access" });
        return;
    }

    const command = await ApplicationCommand.find({ where: { application_id, guild_id } });
    res.send(command);
});
...
router.post(
    ...
    async (req: Request, res: Response) => {
        ...
        const commandExists = await ApplicationCommand.exists({ where: { application_id, guild_id, name: body.name.trim() } });

        if (commandExists) {
            await ApplicationCommand.update({ application_id, guild_id, name: body.name.trim() }, commandForDb);
        } else {
...
router.put(
    ...
    async (req: Request, res: Response) => {
        ...
        const applicationCommands = await ApplicationCommand.find({ where: { application_id, guild_id } });
        ...
        for (const command of commandsNotInArray) {
            await ApplicationCommand.delete({ application_id, guild_id, id: command.id });
        }
        ...
        for (const command of body) {
            ...
            const commandExists = await ApplicationCommand.exists({ where: { application_id, guild_id, name: command.name } });

            if (commandExists) {
                await ApplicationCommand.update({ application_id, guild_id, name: command.name }, commandForDb);
            } else {
                commandForDb.id = Snowflake.generate(); // Have to be done that way so the id doesn't change
                await ApplicationCommand.save(commandForDb);
            }
